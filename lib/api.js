...
route('POST', '/api/patient/kicks', (ctx) => {
  ...
  const hospital = db.hospitals.find((h) => h.id === patient.hospitalId);
  const t = Object.assign({}, clinical.DEFAULT_THRESHOLDS, hospital.thresholds || {});
  ...
  const target = t.kicksMinimumPer12h;
  ...
  const session = {
    ...
    reachedTarget: count >= target,
    createdAt: new Date().toISOString()
  };
  db.kicks.unshift(session);

  const previous = db.kicks.filter((k) => k.patientId === patient.id && k.reachedTarget).slice(1, 6);
  ...
  let message;
  if (count >= target) {
    message = `${count} movements in ${minutes} minute${minutes === 1 ? '' : 's'}, noted. What matters most is whether this feels normal for your baby — if it does not, call your hospital whatever the count says.`;
    // a marked slowdown against her own pattern is worth a nurse's eye
    if (usualMinutes && minutes > usualMinutes * 2 && minutes >= 30) {
      raiseAlert(db, patient, {
        tier: 3,
        reason: 'Movements slower than usual',
        detail: `Took ${minutes} minutes to reach ${target}, against her usual ${usualMinutes}.`,
        source: 'movement counter'
      });
      raised = 1;
      message += ' It took longer than usual, so your hospital has been told.';
    }
  } else if (seconds >= 2 * 3600) {
    raiseAlert(db, patient, {
      tier: 4,
      reason: 'Reduced fetal movements',
      detail: `Only ${count} movements counted in ${minutes} minutes.`,
      source: 'movement counter'
    });
    raised = 1;
    message = `Only ${count} movements in ${minutes} minutes. Call your hospital now — they have been told.`;
  } else {
    message = `${count} movements so far. Keep counting, or start again later when your baby is usually active. If the movements feel fewer or different from usual, do not wait — call your hospital.`;
  }

  store.save();
  return ctx.ok({ session, usualMinutes, target, raised, message });
}, 'patient');

route('GET', '/api/patient/kicks', (ctx) => {
  const db = store.load();
  const list = (db.kicks || []).filter((k) => k.patientId === ctx.user.patientId).slice(0, 20);
  const done = list.filter((k) => k.reachedTarget);
  return ctx.ok({
    sessions: list,
    usualMinutes: done.length ? Math.round(done.reduce((s, k) => s + k.minutes, 0) / done.length) : null,
    target: clinical.DEFAULT_THRESHOLDS.kicksMinimumPer12h
  });
}, 'patient');
