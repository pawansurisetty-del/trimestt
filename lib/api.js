...
route('GET', '/api/patient/insights', (ctx) => {
  ...
  return ctx.ok({
    ...
    countdown: clinical.countdown(patient.lmp),
    consent: patient.consent || null,
    water: Object.assign({}, water, { drunkMl: (todayLog && todayLog.waterMl) || 0 }),
    weight: picture,
...
route('POST', '/api/patient/kicks', (ctx) => {
  ...
  let message;
  if (count >= target) {
    message = `${count} movements in ${minutes} minute${minutes === 1 ? '' : 's'}. That is reassuring.`;
    ...
    message = `${count} movements so far. Keep counting, or start again later when the baby is usually active.`;
  }
