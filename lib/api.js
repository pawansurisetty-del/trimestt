...
/**
 * Four endpoints answer to `TRIMESTT_OWNER_KEY` alone, and one of them resets a
 * hospital administrator's password. There is no second factor and no account
 * behind it, so the key is the entire wall.
 *
 * A short or guessable value is therefore not a smaller version of the same
 * protection — it is no protection, and it is worse than none, because the
 * endpoint looks guarded. `reviewsetup` was set during debugging and reached
 * production. Rather than trust that it gets rotated, the server now refuses to
 * honour a key it can tell is weak, and says so at startup.
 */
const WEAK_OWNER_KEYS = new Set([
  'reviewsetup', 'changeme', 'password', 'secret', 'admin', 'owner',
  'trimestt', 'test', 'testing', 'ownerkey', 'letmein'
]);

const OWNER_KEY_MIN = 24;

function ownerKeyWeakness(key) {
  if (!key) return 'is not set';
  if (WEAK_OWNER_KEYS.has(key.toLowerCase())) return 'is a known placeholder value';
  if (key.length < OWNER_KEY_MIN) return 'is shorter than ' + OWNER_KEY_MIN + ' characters';
  if (/^(.)\1*$/.test(key)) return 'is a single repeated character';
  return null;
}

/**
 * Constant-time compare. The lengths are compared first and separately, which
 * leaks only the length — `timingSafeEqual` throws on a length mismatch, so
 * there is no way to avoid that without hashing both sides, which is not worth
 * it for a key nobody is going to measure across the public internet. The point
 * of using it at all is that `!==` returns on the first differing byte.
 */
...
/** True when the caller proved they hold a *usable* owner key. */
function ownerKeyOk(ctx) {
  const expected = process.env.TRIMESTT_OWNER_KEY || '';
  if (ownerKeyWeakness(expected)) return false;      // weak key disables the door
  const given = ctx.apiKey || clean(ctx.body && ctx.body.ownerKey, 120);
  return sameSecret(given, expected);
}

/* Said once, loudly, at startup — the same place the storage path is checked. */
(function warnAboutOwnerKey() {
  const problem = ownerKeyWeakness(process.env.TRIMESTT_OWNER_KEY || '');
  if (problem) {
    console.warn('[trimestt] owner key ' + problem +
      ' — /api/owner/* endpoints are DISABLED until it is replaced.');
  }
}());
...
const DEFAULT_SETTINGS = {
  reminders: true,          /* visits, scans, medicines */
  alertSound: true,         /* sound on a notification */
  quietFrom: '22:00',       /* nothing but urgent between these hours */
  quietTo: '07:00',
  pageSound: true,          /* the page turn in the book */
  textSize: 'normal',       /* normal | large | largest */
  vibrate: true,            /* buzz with a notification */
  helper: true,             /* the little emergency helper on screen */
  familyShare: false        /* whether her family contact sees reminders */
};

/**
 * Whether her quiet hours are in force right now.
 *
 * Quiet hours never apply to a red alert. They govern reminders — a visit due,
 * a vaccine, a tablet — not the alerts that say something may be wrong. Anything
 * at tier 4 reaches her whatever the hour, and that is deliberate.
 */
function withinQuietHours(patient, at) {
  const set = Object.assign({}, DEFAULT_SETTINGS, patient.settings || {});
  const now = at || new Date();
  const mins = now.getHours() * 60 + now.getMinutes();
  const toMin = (t) => Number(t.slice(0, 2)) * 60 + Number(t.slice(3, 5));
  const from = toMin(set.quietFrom), to = toMin(set.quietTo);
  return from > to ? (mins >= from || mins < to) : (mins >= from && mins < to);
}

/** Whether a notification should actually be delivered to her phone. */
function shouldNotify(patient, tier, at) {
  const set = Object.assign({}, DEFAULT_SETTINGS, patient.settings || {});
  if (tier >= 4) return true;                 // urgent always gets through
  if (!set.reminders) return false;
  return !withinQuietHours(patient, at);
}
...
function motherPayload(db, patient) {
  ...
  return {
    ...
    altPhone: patient.altPhone || null,
    attendantName: patient.attendantName || null,
    attendantPhone: patient.attendantPhone || null,
    attendantRelation: patient.attendantRelation || null,
    settings: Object.assign({}, DEFAULT_SETTINGS, patient.settings || {}),
...
route('POST', '/api/owner/hospital-reset', (ctx) => {
  if (!ownerKeyOk(ctx)) return ctx.fail(401, 'Not authorised.');
...
/** Create or refresh the app-store reviewer account, over HTTPS.
    `railway run` executes on your own machine, so a script cannot reach the
    production volume; this can. */
route('POST', '/api/owner/reviewer-account', (ctx) => {
  if (!ownerKeyOk(ctx)) return ctx.fail(401, 'Not authorised.');

  const db = store.load();
  const PASSWORD = clean(ctx.body.password, 40) || 'AppReview2026';
  ...
  if (!hospital) {
    ...
    db.hospitals.push(hospital);
    db.users.push({
      id: store.id('usr'), role: 'hospital', staffRole: 'admin',
      hospitalId: hospital.id, name: 'App Review',
      email: 'review@trimestt.com',
      passwordHash: auth.hashPassword(PASSWORD),
      createdAt: new Date().toISOString()
    });
  }
  ...
  const ago = (n) => {
    const d = new Date(); d.setDate(d.getDate() - n); return clinical.iso(d);
  };
  ...
  if (!patient) {
    ...
    const lmp = ago(168);
    patient = {
      id: store.id('pat'), hospitalId: hospital.id,
      number: 'TRM-REV01-' + String(seq).padStart(4, '0'),
      name: 'Demo Patient', phone: '9000000000', age: 28,
      lmp, edd: clinical.eddFromLmp(lmp),
      bloodGroup: 'O positive', consultant: 'Dr Demo',
      heightCm: 160, prePregnancyWeightKg: 55, riskTags: [],
      activated: true, activationCode: auth.activationCode(), completed: {},
      medicines: [
      ...
      consent: { agreed: true, version: TERMS_VERSION, at: new Date().toISOString() },
      createdAt: new Date().toISOString()
    };
    db.patients.push(patient);
    hospital.credits.used += 1;
    ...
    for (let d = 20; d >= 0; d--) {
      db.logs.unshift({
        ...
        systolic: 112 + (d % 5), diastolic: 72 + (d % 4),
        kicks: 10 + (d % 3), waterMl: 1800 + (d % 4) * 200,
        symptoms: [], medicinesTaken: true,
        createdAt: new Date().toISOString()
      });
    }
  }

  /* login is by patient number, so the user record must carry it */
  let user = db.users.find((u) => u.role === 'patient' && u.patientId === patient.id);
  if (!user) {
    user = {
      id: store.id('usr'), role: 'patient',
      patientId: patient.id, hospitalId: hospital.id,
      number: patient.number, name: patient.name,
      passwordHash: auth.hashPassword(PASSWORD),
      createdAt: new Date().toISOString()
    };
    db.users.push(user);
  } else {
    user.number = patient.number;
    user.passwordHash = auth.hashPassword(PASSWORD);
  }
  ...
  return ctx.ok({
    ...
    password: PASSWORD,
    hospitalEmail: 'review@trimestt.com',
    gestation: clinical.gestation(patient.lmp).label,
    logs: db.logs.filter((l) => l.patientId === patient.id).length,
    note: 'Paste the patient ID and password into App Review notes and Play test instructions.'
  });
});
...
route('GET', '/api/patient/settings', (ctx) => {
  const db = store.load();
  const patient = db.patients.find((p) => p.id === ctx.user.patientId);
  return ctx.ok({
    settings: Object.assign({}, DEFAULT_SETTINGS, patient.settings || {}),
    hasPhoto: !!patient.photo,
    devices: (patient.devices || []).length
  });
}, 'patient');

route('POST', '/api/patient/settings', (ctx) => {
  const db = store.load();
  const patient = db.patients.find((p) => p.id === ctx.user.patientId);
  const now = Object.assign({}, DEFAULT_SETTINGS, patient.settings || {});
  const b = ctx.body || {};

  ['reminders', 'alertSound', 'pageSound', 'vibrate', 'helper', 'familyShare'].forEach((k) => {
    if (b[k] !== undefined) now[k] = !!b[k];
  });
  if (/^\d\d:\d\d$/.test(String(b.quietFrom || ''))) now.quietFrom = b.quietFrom;
  if (/^\d\d:\d\d$/.test(String(b.quietTo || ''))) now.quietTo = b.quietTo;
  if (['normal', 'large', 'largest'].indexOf(b.textSize) > -1) now.textSize = b.textSize;

  patient.settings = now;
  store.save();
  return ctx.ok({ settings: now });
}, 'patient');

/** She can change her own password without troubling the hospital. */
route('POST', '/api/patient/password', (ctx) => {
  const db = store.load();
  const user = db.users.find((u) => u.id === ctx.user.id);
  if (!auth.verifyPassword(String(ctx.body.current || ''), user.passwordHash)) {
    return ctx.fail(401, 'Your current password is wrong.');
...
/* ---- devices that can receive a push ------------------------------------ */

route('POST', '/api/patient/device', (ctx) => {
  const db = store.load();
  const patient = db.patients.find((p) => p.id === ctx.user.patientId);
  const token = clean(ctx.body.token, 300);
  if (!token) return ctx.fail(400, 'No device token.');

  patient.devices = (patient.devices || []).filter((d) => d.token !== token);
  patient.devices.unshift({
    token,
    platform: clean(ctx.body.platform, 20) || 'unknown',
    registeredAt: new Date().toISOString()
  });
  patient.devices = patient.devices.slice(0, 5);   // she may change phones
  store.save();
  return ctx.ok({ devices: patient.devices.length });
}, 'patient');

route('GET', '/api/hospital/patients/:id/devices', (ctx) => {
  const db = store.load();
  const patient = db.patients.find((p) => p.id === ctx.params.id && p.hospitalId === ctx.user.hospitalId);
  if (!patient) return ctx.fail(404, 'Patient not found.');
  return ctx.ok({
    devices: (patient.devices || []).map((d) => ({ platform: d.platform, registeredAt: d.registeredAt })),
    reachable: (patient.devices || []).length > 0
  });
}, 'hospital');
...
route('POST', '/api/patient/profile', (ctx) => {
  const db = store.load();
  const patient = db.patients.find((p) => p.id === ctx.user.patientId);

  /* Her name, dates and clinical details stay with the hospital. These are the
     contact details she should be able to correct herself. */
  const phone = clean(ctx.body.altPhone, 20);
  if (phone && !/^[0-9+\s-]{10,15}$/.test(phone)) {
    return ctx.fail(400, 'That does not look like a phone number.');
  }
  const attendantPhone = clean(ctx.body.attendantPhone, 20);
  if (attendantPhone && !/^[0-9+\s-]{10,15}$/.test(attendantPhone)) {
    return ctx.fail(400, 'That does not look like a phone number.');
  }

  patient.altPhone = phone;
  patient.attendantName = clean(ctx.body.attendantName, 120);
  patient.attendantPhone = attendantPhone;
  patient.attendantRelation = clean(ctx.body.attendantRelation, 40);
  store.save();
  ...
  return ctx.ok({
    ...
    attendantName: patient.attendantName,
    attendantPhone: patient.attendantPhone,
    attendantRelation: patient.attendantRelation
  });
}, 'patient');
...
route('POST', '/api/patient/withdraw-consent', (ctx) => {
  ...
  return ctx.ok({
    withdrawn: true,
    message: 'Your agreement is withdrawn and this is recorded for your hospital. Your medical records stay with the hospital, as the law requires. You can agree again any time with a new code from them.'
  });
}, 'patient');
...
route('GET', '/api/files/:name', (ctx) => {
  ...
  /* A profile picture belongs to the patient directly — it is not a record and
     not a log, which is why it uploaded fine and then would not display. */
  const portrait = db.patients.find((pt) => pt.photo === name);
  const owner = record || log || portrait;
  if (!owner) return ctx.fail(404, 'Not found.');

  const ownerPatientId = portrait ? portrait.id : owner.patientId;
  const allowed = ctx.user && (
    (ctx.user.role === 'patient' && ownerPatientId === ctx.user.patientId) ||
...
route('POST', '/api/patient/logs', (ctx) => {
  ...
  return ctx.ok({
    ...
    message: raised.length
      ? 'This is flagged on your record for your hospital. If you feel unwell, call them now \u2014 do not wait for them to see it.'
      : 'Saved.'
  });
}, 'patient');
...
route('POST', '/api/patient/sos', (ctx) => {
  ...
  return ctx.ok({
    ...
    /* This used to say "Your consultant is being paged." Nothing pages anyone.
       raiseAlert() writes a row to db.alerts and returns; there is no SMS, no
       email, no push (the Firebase plugin is present but unconfigured) and the
       staff dashboard does not poll. The hospital sees this when a member of
       staff next opens the dashboard — which at three in the morning is nobody.
       Telling a woman in labour that help is already moving, when it is not,
       could stop her making the call that actually reaches someone. The
       phone call is the real mechanism, so the words now say so. */
    instruction: payload.preterm
      ? 'Call the hospital now — that is what reaches them fastest. Then get ready to leave. Your details have been saved to your record for the staff to see.'
      : 'Call the hospital now — that is what reaches them fastest. Then get ready to leave. Your details have been saved to your record for the staff to see.'
  });
}, 'patient');
...
route('POST', '/api/patient/children/:id/growth', (ctx) => {
  ...
  return ctx.ok({
    ...
    message: raised ? 'This is flagged on your record for your hospital. If you are worried, call them now \u2014 do not wait for them to see it.' : 'Saved.'
  });
}, 'patient');
...
module.exports = { handle, MOTHER_FEE, CHILD_FEE, withinQuietHours, shouldNotify, DEFAULT_SETTINGS };
