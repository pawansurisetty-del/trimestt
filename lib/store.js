...
/* ------------------------------------------------------------ postgres -- */
/**
 * When DATABASE_URL is set and `pg` is installed, every call below is served by
 * lib/store-pg.js instead — same functions, same shapes, the document kept in a
 * Postgres row rather than a file on the volume. Nothing else in the app knows
 * the difference.
 *
 * When it is not, this file behaves exactly as it always has. A missing
 * variable therefore degrades to the old behaviour rather than failing to boot,
 * which matters because the JSON store is still the supported path for a single
 * hospital.
 *
 * Read the header of store-pg.js before assuming this makes the app
 * horizontally scalable. It does not — it is still one writer.
 */
const pg = require('./store-pg');

if (pg.available()) {
  module.exports = pg.create();
} else {
  if (process.env.DATABASE_URL) {
    console.warn('[trimestt] DATABASE_URL is set but the "pg" package is not installed ' +
                 '— falling back to the JSON file. Run: npm install pg');
  }
  /* init() exists on both paths so server.js can await it unconditionally. */
  module.exports = {
    init: async () => {},
    load, save, saveNow, nextSeq, id, reset, DB_FILE
  };
}
