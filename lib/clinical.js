...
function weightPicture(patient, logs) {
  /* Sort by day, then by when it was written.
   *
   * `date` is a day only — clean(b.date, 10) — so every entry logged today
   * carries an identical key. Array.sort is stable and `logs` arrives newest
   * first, so same-day entries kept that order and taking the last one returned
   * the OLDEST of the day. She would log a new weight, it saved correctly, and
   * the card went on showing the first figure she had entered that morning.
   * Nothing looked broken; the number simply never moved.
   *
   * createdAt is a full timestamp, so it breaks the tie properly. Entries
   * written before that field existed fall back to the id, which is
   * time-ordered, and finally to the day itself. */
  const at = (l) => l.createdAt || l.id || l.date;
  const weighed = (logs || []).filter((l) => l.weight).sort((a, b) => {
    const day = a.date.localeCompare(b.date);
    return day !== 0 ? day : String(at(a)).localeCompare(String(at(b)));
  });
