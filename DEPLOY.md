...
### Check the storage path on every deploy

The service prints one line at startup:

```
[trimestt] storage: /data
```

If it says anything else, or carries the warning that `TRIMESTT_DATA` is not
set, **the database is inside the container and the next deploy will destroy
it**. Stop and fix the variable before letting anyone use the app.

### Using your own journey illustration

Drop a file named **`journey-mother.png`** into `public/`. The home banner uses it
automatically; without it, the drawn figure is shown instead, so the banner is
never empty either way.

- About **420 x 620 px**, or larger. Anything under 300 px wide will look soft on
  a modern phone
- Transparent background, or a background matching the banner's soft pink
- The figure should sit against the right edge, feet at the bottom
- Make sure you hold the rights to it. An illustration lifted from a design
  mockup or a stock site is a real risk in a commercial medical product
